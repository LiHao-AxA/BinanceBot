    ...
    try:
        ...
        while True:
            ...
            if latest_filled_order:
                entry_price = float(latest_filled_order['price'])
                ...
                for open_order in open_orders:
                    try:
                        ...
                        logger.warning(f"取消订单{order['orderId']}时出现异常：{e}") 
            ...
            if grid_status == 1:
                ...
                while not order_filled:
                    ...
                    latest_filled_order = max(filled_orders, key=lambda x: x['updateTime'], default=None)
                    if latest_filled_order:
                        new_entry_price = float(latest_filled_order['price'])
                        if new_entry_price != entry_price:
                            order_filled = True
                            logger.info(f"订单已成交，新的成交价为{new_entry_price}")
                    time.sleep(2)  # 每隔5秒查询一次订单状态        

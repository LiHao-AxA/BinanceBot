...
for balance in balances:
    if balance['asset'] == coin:
        # print(balance)
        coin_balance = float(balance['withdrawAvailable'])
        logger.info(f"coin余额: {coin_balance}")
        # print(coin_balance)
leverage = 10  # 设置杠杆大小
# 设置杠杆
client.futures_coin_change_leverage(symbol=symbol, leverage=leverage)
...
logger.info(f"开仓精度:{contract_size}")
print(f"开仓精度:{contract_size}")
# 价格精度
...
# coin 余额价值换算 USD
coin_value_in_usd = round((coin_balance * current_price),0)
print(f"价值：{coin_value_in_usd}USD")
# 换算可开
max_position = coin_value_in_usd  * leverage / contract_size
print(f"可开: {max_position}张")
# 设置USDT初始余额
usdt_balance_status = 60
# 设置COIN初始余额
coin_balance_status = round((usdt_balance_status / contract_size),price_precision)
logger.info(f"{coin}初始余额: {coin_balance_status}")
# USDT设置初始仓位
usdt_position_size = 3
# COIN设置初始仓位
position_size = round(( usdt_position_size * leverage / current_price),0)# 初始仓位为0.01张
logger.info(position_size)
# USDT设置网格仓位
usdt_grid_spacing = 1
# COIN设置网格加仓数量
grid_spacing = round(usdt_grid_spacing * leverage / current_price * (coin_balance / coin_balance_status),0) # 每个网格的加仓数量为3个网格间距
...
if grid_status == 1:   
    if entry_price == 0:
        ticker = client.futures_coin_symbol_ticker(symbol=symbol)
        entry_price = float(ticker[0]['price'])
        logger.info(f"初始开多仓价格为，以当前市价{entry_price}开仓")
    ...
    if entry_price == 0: 
        ticker = client.futures_coin_symbol_ticker(symbol=symbol)
        entry_price = float(ticker[0]['price'])
        logger.info(f"初始开空仓价格为，以当前市价{entry_price}开仓")
...
while True:
    try:
        ...
        while True:
            # 获取历史最新成交价
            orders = client.futures_coin_get_all_orders(symbol=symbol)       
            ...
            if latest_filled_order:
                ...
                for open_order in open_orders:
                    client.futures_coin_create_order(symbol=symbol, orderId=open_order['orderId'])   
